...
-- publisher
CREATE TABLE IF NOT EXISTS `cg_publisher` (
  `id` int PRIMARY KEY NOT NULL AUTO_INCREMENT,
  `name` varchar(255) NOT NULL,
  `created_at` datetime NOT NULL DEFAULT (now()),
  `updated_at` datetime NOT NULL DEFAULT (now())
...
-- book publisher
CREATE TABLE IF NOT EXISTS `cg_book_publisher` (
  `id` int PRIMARY KEY AUTO_INCREMENT,
  `book_id` int,
  `publisher_id` int,
  `created_at` datetime NOT NULL DEFAULT (now()),
...
ALTER TABLE `cg_book_publisher` ADD FOREIGN KEY (`book_id`) REFERENCES `cg_books` (`id`) ON DELETE CASCADE;
ALTER TABLE `cg_book_publisher` ADD FOREIGN KEY (`publisher_id`) REFERENCES `cg_publisher` (`id`) ON DELETE CASCADE;
